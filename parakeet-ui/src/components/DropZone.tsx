...
export default function DropZone({ onFile, disabled }: Props) {
  ...
  return <label className={`drop-zone ${drag ? 'dragging' : ''} ${picked ? 'picked' : ''} ${disabled ? 'disabled' : ''}`}
    ...
    {picked ? <div className="drop-copy"><strong><FileAudio size={15} />{picked.name}</strong><span>{fmt(picked.size)} · wybierz inny plik</span></div> :
      <div className="drop-copy"><strong>Upuść plik lub wybierz z dysku</strong><span>MP3, WAV, MP4, MKV, M4A, FLAC, OGG, WEBM</span></div>}
