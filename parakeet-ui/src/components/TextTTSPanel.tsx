...
import { ApiRequestError, listSpeakers, listTTSModels, streamJob, submitTextTTS } from '@/lib/api';
...
export default function TextTTSPanel() {
  ...
  const run = async () => {
    setRunning(true); setError(''); setQuotaExceeded(false); setAudioUrl(''); setProgress(0); setMessage('Przygotowuję syntezę…');
    try {
      const id = await submitTextTTS({
        text, speaker_label: speaker, tts_model_profile: ttsModel, language: lang,
        speed, dur_scale: 1, ...flowSettings, digital_silence: true, pause_edge_frames: 10,
      ...
      await new Promise<void>((resolve, reject) => streamJob(id, ev => {
        if (ev.type === 'progress') { setProgress(ev.progress ?? 0); setMessage(ev.message ?? ''); }
          ...
          setDuration(result.duration); setAudioUrl(`/jobs/${id}/audio`);
  ...
  return <div className="voice-studio-grid">
    ...
    <aside className="panel voice-controls">
      <label><span className="field-label">{t('speaker')}</span><select value={speaker} onChange={e => setSpeaker(e.target.value)}>{speakers.map(item => <option key={`${item.id}-${item.label}`} value={item.label}>{item.display_name ?? item.label}</option>)}</select></label>
